...
function createBMP(pattern, size = 24) {
  const width = size;
  const height = size;
  const headerSize = 54;
...
for (const [name, pattern] of Object.entries(navIcons)) {
  const buf = createBMP(pattern.trim(), 24);
  fs.writeFileSync(path.join(outDir, `${name}.bmp`), buf);
}
console.log('Nav BMP icons 24x24 created successfully! Silakan buka di Aseprite.');
